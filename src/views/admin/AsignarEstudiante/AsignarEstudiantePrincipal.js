...
export default function AsignarEstudiantePrincipal() {
  ...
  useEffect(() => {
    ...
    const fetchEstudiantes = async () => {
      try {
        ...
        console.log('estudiante', response.data.results)
  ...
  const handleVerCuotas = (idEstudiante) => {
    history.push(`/admin/CuotaEstudiante/CuotaEstudiantePrincipal/${idCiclo}/jornada/${idJornadaCiclo}/grado/${idGrado}/estudiante/${idEstudiante}`)
  };
                ...
                <tr key={estudiante.id_asignacion}>
                  ...
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                    <button
                      ...
                      onClick={() => handleVerCuotas(estudiante.id_estudiante)}
                    >
                      <i className="fas fa-star mr-2"></i>
                      {estudiante.cantidad_cuotas}
                    </button>
                  </td>
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                    <button
